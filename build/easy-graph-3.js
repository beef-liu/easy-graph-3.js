...
EG3.GridPositionDistribution = function(gridWidth) {
    ...
    this.reset = function() {
        ...
        _curRoundGrids.push(new THREE.Vector3(0, 0, 0));
        
        _roundEndPos = new THREE.Vector3(0, 0, 0);
    };
    ...
    function createRoundPositions(d, step) {
        ...
        x += step;
        y -= step;
        _curRoundGrids.push(new THREE.Vector3(x, y, z));
...
EG3.SpherePositionDistribution = function(originPoint, direction, maxCount, space, fov) {
    ...
    this.reset = function() {
        ...
        this.axisEndPoint = new THREE.Vector3(
            originPoint.x + this.radius * this.axisUnitVector.x, 
            originPoint.y + this.radius * this.axisUnitVector.y, 
            originPoint.z + this.radius * this.axisUnitVector.z
            );
        ...
        _curRoundPositions.push(new THREE.Vector3(this.axisEndPoint.x, this.axisEndPoint.y, this.axisEndPoint.z));
    };
    ...
    function createRoundPositions(R, r, splitCount) {
        ...
        for(i = 0; i < splitCount; i++) {
            _curRoundPositions.push(new THREE.Vector3(
                r * Math.cos(alpha),
                r * Math.sin(alpha),
                Math.pow(R*R - r*r, 0.5)
            ));
 ...
 *      lineType: String,                                               //optional. (default: "arrow") could be "line", "arrow"
 ...
 *      pointType: String,                                              //optional. (default: "sphere") could be "point", "sphere"
...
EG3.NetworkGraph = function(args) {
    ...
    if(args.pointType != undefined) {
        this.pointType = args.pointType;
    }
    ...
    */
    ...
    this.reloadGraphNodes = function(graphNodes) {
        ...
        } else if(_pointType == "point") {
            _o3dPoints = new THREE.PointCloud(new THREE.Geometry(), new THREE.PointCloudMaterial({color: this.pointColor, size: _pointRadius}));
        }
        
        //_matLine = new THREE.LineBasicMaterial({color: this.lineColor});
        if(_lineType == "arrow") {
            _o3dLines = new THREE.Object3D();
        } else if(_lineType == "line") {
            _o3dLines = new THREE.Line(new THREE.Geometry(), new THREE.LineBasicMaterial({color: this.lineColor}), THREE.LinePieces);
    ...
    function addPoint(position, radius, graph) {
        if(_pointType == "sphere") {
            //create ball ------------                        
            ...
            _o3dPoints.geometry.vertices.push(new THREE.Vector3(position.x, position.y, position.z));

            //callback                    
            if(_callbackAfterPointAdded) {
                _callbackAfterPointAdded(_o3dPoints, graph);
            }
        }
    ...
    function addLine(startPoint, endPoint) {
        ...
        if(_lineType == "arrow") {
            ...
            _o3dLines.geometry.vertices.push(startPoint.clone(), endPoint.clone());
        }
    }
...
/*
    function makeGraphNodesNoRecursion_old(graphNodes, gridSphereCenter) {
    ...
    */
    ...
    function makeGraphNodesNoRecursion(graphNodes, gridSphereCenter) {
        var allGraphNodesMap = new EG3.Map();
        var graphNodesBuffer = graphNodes.slice(0, graphNodes.length);
        //var unrenderedGraphNodesMap = new EG3.Map();
        var renderedNodePosMap = new EG3.Map();
        ...
        var rootPosDist;
        {
            newGridSphereCenter = gridSphereCenter;  
            var newGridSphereRadius = 1000;
            var newGridSphereSurfaceCenterDir = newGridSphereCenter.clone().sub(_childrenFocusPosition).normalize();
            var newGridSphereSurfaceCenter = newGridSphereSurfaceCenterDir.multiplyScalar(newGridSphereRadius).add(newGridSphereCenter);
            rootPosDist = new EG3.GridSpherePositionDistribution(_gridWidth, newGridSphereCenter, newGridSphereSurfaceCenter);
        }

        var graph;
        var thisPos;
        while((graph = graphNodesBuffer.shift())) {
            //check if rendered
            /*
            if(unrenderedGraphNodesMap.get(graph.vid) == undefined) {
                continue;
            ...
            */
            ...
            thisPos = renderedNodePosMap.get(graph.vid); 
            if(thisPos == undefined) {
                //thisPos = posDist.nextPosition();
                thisPos = rootPosDist.nextPosition();
                renderedNodePosMap.set(graph.vid, thisPos);
            ...
            if(graph.children.length > 0) {
                var posDist;
                {
                    newGridSphereCenter = thisPos;  
                    var newGridSphereRadius = Math.max(
                        makeGridSphereRadius(graph.children.length, _gridWidth, _gridRadianMax), 
                        _gridWidth * 2);
                    var newGridSphereSurfaceCenterDir = newGridSphereCenter.clone().sub(_childrenFocusPosition).normalize();
                    var newGridSphereSurfaceCenter = newGridSphereSurfaceCenterDir.multiplyScalar(newGridSphereRadius).add(newGridSphereCenter);
                    posDist = new EG3.GridSpherePositionDistribution(_gridWidth, newGridSphereCenter, newGridSphereSurfaceCenter);
